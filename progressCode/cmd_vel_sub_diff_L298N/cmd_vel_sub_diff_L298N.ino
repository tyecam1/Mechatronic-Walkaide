...
void setup() {
  ...
  nh.subscribe(sub);
  Serial.println(F("Waiting for goal coordinates..."));
}

void loop() {
  nh.spinOnce();  // Listen for messages
  delay(100);  // Small delay to allow for incoming ROS messages
}
...
void turnToFaceDirection(float goal_x, float goal_y) {
  ...
  Serial.print(F("Turning to Face: "));
  ...
  while (abs(angle_diff) > 5) {
    ...
    Serial.print(F("Current Heading Published: "));
