...
def create_anim_df(df_path, nav_df):
    """Merge odometry and navigation force data for animation."""
    forces_list = []
...
def birdseye_plot(df_path, obstacles_df, title, results_folder):
    """Generate a birdseye plot of the path and obstacles; return filtered obstacles and goal center."""
...
def compute_and_record_reaction_times(report_name, arduino_df, nav_df, output_csv="reaction_times.csv"):
    """Calculate average reaction times from command and navigation data and record results."""
...
def process_bag_file(bag_file):
    ...
    updated_obstacles, final_goal_center = birdseye_plot(df_path, obstacles_df, title_interactive, results_folder)
...
if __name__ == "__main__":
    bag_files = glob.glob(os.path.join("room1bagfiles", "*.bag"))
