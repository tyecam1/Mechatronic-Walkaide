...
def compute_apf_command():
    ...
    k_rep = 15
    d0 = 2.0  # influence distance (meters)
    F_rep_x = 0.0
    F_rep_y = 0.0
    for obs in obstacles:
        ...
        obs_volume = (obs_size[0] * obs_size[1] * obs_size[2]) if obs_size[0] and obs_size[1] and obs_size[2]  else 1.0
        ...
        if 0 < dist < d0:
            rospy.loginfo("Obstacle at (%.2f, %.2f) with distance: %.2f and volume %.2f", obs_x, obs_y, dist, obs_volume)
            rep_mag = k_rep * obs_volume * (1.0/dist - 1.0/d0) / (dist**2)
